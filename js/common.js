// Common JS (chatbot, search, nav active)
...
function sendQuickMessage(message){ const i=document.getElementById('chatbotInput'); if(i){ i.value=message; sendMessage(); } }
function addMessage(content, sender){
  const c = document.getElementById('chatbotMessages'); if(!c) return;
  const m = document.createElement('div'); m.className = `message ${sender}`;
  const a = document.createElement('div'); a.className='message-avatar'; a.textContent = sender==='bot'?'🤖':'👤';
  const b = document.createElement('div'); b.className='message-content'; b.innerHTML = content;
  m.appendChild(a); m.appendChild(b); c.appendChild(m); c.scrollTop = c.scrollHeight;
}
function showTypingIndicator(){
  const c = document.getElementById('chatbotMessages'); if(!c) return;
  const t = document.createElement('div'); t.className='message bot'; t.id='typingIndicator';
  const a = document.createElement('div'); a.className='message-avatar'; a.textContent='🤖';
  const tc = document.createElement('div'); tc.className='typing-indicator'; tc.style.display='flex';
  tc.innerHTML = '<div class="typing-dot"></div><div class="typing-dot"></div><div class="typing-dot"></div>';
  t.appendChild(a); t.appendChild(tc); c.appendChild(t); c.scrollTop = c.scrollHeight;
}
function hideTypingIndicator(){ const t=document.getElementById('typingIndicator'); if(t) t.remove(); }
function handleChatbotKeyPress(event){ if(event.key==='Enter') sendMessage(); }

function generateBotResponse(userMessage){
  const m = userMessage.toLowerCase();
  if (m.includes('ga4') || m.includes('google analytics')){
    if (m.includes('설치') || m.includes('설정')) return '<strong>GA4 설치 방법</strong><br><br>1. <a href="ga-setup.html">GA4 계정 생성</a><br>2. 속성 및 데이터 스트림 설정<br>3. gtag.js 코드 설치<br>4. GTM 설치 (권장)';
    if (m.includes('이벤트') || m.includes('추적')) return '<strong>GA4 이벤트 추적</strong><br><br><a href="ga-events.html" style="color:#E60012;">이벤트 설정 가이드 →</a>';
    if (m.includes('전자상거래') || m.includes('이커머스')) return '<strong>전자상거래 추적</strong><br><br><a href="ga-ecommerce.html" style="color:#E60012;">전자상거래 가이드 →</a>';
    return '<strong>GA4 소개</strong><br><br><a href="ga-intro.html" style="color:#E60012;">GA4 소개 보기 →</a>';
  } else if (m.includes('gtm') || m.includes('tag manager')){
    if (m.includes('설치') || m.includes('설정')) return '<strong>GTM 설치</strong><br><br><a href="gtm-setup.html" style="color:#E60012;">GTM 설치 가이드 →</a>';
    if (m.includes('운영') || m.includes('관리')) return '<strong>GTM 운영</strong><br><br><a href="gtm-operation.html" style="color:#E60012;">운영 가이드 →</a>';
    if (m.includes('데이터 레이어')) return '<strong>데이터 레이어</strong><br><br><a href="gtm-datalayer.html" style="color:#E60012;">데이터 레이어 가이드 →</a>';
    return '<strong>GTM 소개</strong><br><br><a href="gtm-intro.html" style="color:#E60012;">GTM 소개 보기 →</a>';
  } else if (m.includes('전환') || m.includes('conversion')){
    return '<strong>GA4 전환</strong><br><br><a href="ga-conversion.html" style="color:#E60012;">전환 설정 가이드 →</a>';
  } else if (m.includes('잠재고객') || m.includes('audience')){
    return '<strong>GA4 잠재고객</strong><br><br><a href="ga-audience.html" style="color:#E60012;">잠재고객 가이드 →</a>';
  } else if (m.includes('안녕') || m.includes('hello') || m.includes('hi')){
    return '안녕하세요! 👋<br><br>GA & GTM 가이드 AI입니다. 궁금한 내용을 질문해주세요.';
  } else if (m.includes('도움') || m.includes('help')){
    return '<strong>도움말</strong><br><br>GA4 설치/이벤트/전자상거래, GTM 설치/운영/데이터 레이어를 지원합니다.';
  }
  return '죄송합니다. 더 구체적인 키워드로 질문해주세요. 예) "GA4 설치", "GTM 설정"';
}
